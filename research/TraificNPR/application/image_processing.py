...
def process_frame(frame, frame_number, filename_prefix, plate_model, seg_model, recog_model, device, ocr_font):
    ...
    if not plate_results or not len(plate_results[0].boxes):
        logging.debug(f"No plates detected in frame {frame_number} ({filename_prefix}). Fallback to whole image.")
        class MockTensor:
        ...
        class MockBox:
            def __init__(self, w, h):
                self.conf = [1.0]
                self.xyxy = [MockTensor([0, 0, w, h])]
        detected_boxes = [MockBox(w_frame, h_frame)]
    else:
